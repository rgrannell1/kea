	...
	message('test that every method has an unchaining version.')
		...
		base_methods_names <- gsub('_', '', kiwi $ functions)
		...
		xMap(
			xUnspread((method : proto_name) := {
				...
				if (method != 'x') {
					...
					params <- xParamsOf(getNamespace('kiwi')[[method]])
					...
					is_exception <- method %in% implement_exception[[proto_name]]

					if (proto_name %in% expected_proto && (method %!in% proto) && !is_exception) {

						message <- invoking_call %+% ': the method ' %+% method %+% ' should be in the prototype ' %+% proto_name %+%
							' but was not.'

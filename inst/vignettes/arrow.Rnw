...
\paragraph{}
Arrow wraps low-level functions in R to add immutable values to R. The current syntax for
creating values is
...
though this may change in the future. Any attempts to edit a value results in an error.
Conceptually values create an symbolic alias to a value, and an attempt to modify a value makes
as much sense as the assignment $1\ =\ 2$

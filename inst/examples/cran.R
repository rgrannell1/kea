...
x_(r_source)$
xMap(package := {
	#--- return the swears in a package.

	package <- unlist(package)

	x_(swears)$
	xFlatMap(swear := {
		if ( any(grepl(swear, package)) ) {
			swear
		}
	})$x()
...
xReject(xIsEmpty)$
xMap(package:= {
	#--- count occurences in each package.
	as.list(table(package))
})$
xReducel(
	function (acc, new) {
		#--- count total occurrences in this package.

		if (length(names(new)) > 0 && names(new) %in% names(acc)) {
			acc[[ names(new) ]] <- 
				acc[[ names(new) ]] + new[[1]]
			acc
		} else {
